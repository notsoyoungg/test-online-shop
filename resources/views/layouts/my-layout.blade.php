    ...
    <div class="container d-flex flex-column flex-md-row align-items-center justify-content-between gap-2 gap-md-1">
        ...
        @if(auth()->user())
            <div class="d-flex flex-column align-items-center">
                <span class="bg-primary fs-6 text-light px-2 rounded-pill">{{ number_format((auth()->user()->bonuses ?? 0), 0, '.', ' ') }} баллов</span>
                <span class="login text-light fs-6">{{ Auth::user()->name }}</span>
            </div>
        @endif

        <div>
            <a class="btn" href="{{ route('show-cart') }}" title="Корзина">
            ...
            @if(auth()->user())
                ...
                <button class="btn" form="logout-form" title="Выйти">
                    <svg width="35px" height="35px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
